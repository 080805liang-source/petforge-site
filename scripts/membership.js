...
function setMode(nextMode) { mode = nextMode; modeButtons.forEach((button) => button.classList.toggle("is-active", button.dataset.memberMode === mode)); authSubmit.innerHTML = `${mode === "login" ? "登录" : "注册"} <span>→</span>`; confirmPasswordField.hidden = mode !== "signup"; confirmPasswordInput.required = mode === "signup"; smsField.hidden = mode !== "signup" || identityMode !== "phone"; authNote.textContent = ""; }
function setIdentityMode(nextMode) { identityMode = nextMode; identityButtons.forEach((button) => button.classList.toggle("is-active", button.dataset.identityMode === identityMode)); emailField.hidden = identityMode !== "email"; phoneField.hidden = identityMode !== "phone"; emailInput.required = identityMode === "email"; phoneInput.required = identityMode === "phone"; smsField.hidden = mode !== "signup" || identityMode !== "phone"; }
